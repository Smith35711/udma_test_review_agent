...
│       ├── log.py              # 日志初始化与配置
│       ├── llm.py              # 模型构造与调用封装
│       ├── nodes/              # 节点子包：llm/ 大模型节点、tool/ 确定性工具节点
│       ├── tools/              # 工具子包（接口与实现分离）
│       │   ├── __init__.py     # 工具聚合导出
│       │   ├── registry.py     # @tool 接口定义与描述
│       │   └── executors/      # 复杂执行实现
│       │       └── script_runner.py  # 脚本/命令执行器
│       ├── prompts/            # 提示词子包（按节点拆分，每个文件一个节点）
│       └── graph.py            # 图组装与编译入口
...
| `prompts/` | 提示词子包：按节点拆分为独立模块，每模块导出该节点的模板常量；引用方直接导入对应子模块 | Prompt 模板字符串 |
...
| `nodes/` | 实现各节点：以 `(state) -> dict | Command` 签名编写；按性质分为 `llm/`（大模型节点）与 `tool/`（确定性工具节点），公共辅助置于 `common.py` | 节点函数 |
...
- 图结构与业务逻辑分离：`graph.py` 只做组装，节点实现下沉至 `nodes/`（大模型节点与确定性工具节点分目录）。
